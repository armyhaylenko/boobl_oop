...
int main() {
    ...
    cout << t1.get_remaining_time() << " millis left until t1 fires." << endl; // should print around 1000

    Timer t2(5000);
    t2.start();
    this_thread::sleep_for(milliseconds(1000));
    cout << t2.get_remaining_time() << " millis left until t2 fires." << endl; // should print around 4000

    auto now = system_clock::now();

    Watch w1(duration_cast<seconds>(now.time_since_epoch()).count() + 10); // trigger in 10 seconds
    w1.start();
    cout << "w1 will fire on " << w1.get_readable_alarm_time() << endl;
    this_thread::sleep_for(milliseconds(5000));
    cout << w1.get_remaining_time() << " seconds left until w1 fires." << endl; // 10 - 5 = 5 seconds until firing
